...
t("com so o ponto da ocorrencia desenha o PCO", () => {
  const svg = ev('(function(){ O=novoEstado(); O.meta.lat="41.27"; O.meta.lon="-7.47"; return croquiSVG(); })()');
  ok(/<svg/.test(svg) && /PCO/.test(svg), svg.slice(0, 120));
});
...
t("limpar o perimetro apaga a geometria mas guarda o ponto", () => {
  w.document.getElementById("p-limpar").click();
  ig(ev("O.dados.perim"), null, "a geometria devia sair:");
  /* o croqui continua, com o ponto da ocorrencia: e o que ha para mostrar */
  ig(w.document.getElementById("croqui-box").style.display, "block");
  /* o marcador do PCO tambem e um <path d="M...>: o poligono distingue-se pelo preenchimento */
  ok(!/fill-opacity="\.16"/.test(w.document.getElementById("croqui-svg").innerHTML), "o poligono ficou desenhado");
  ok(/PCO/.test(w.document.getElementById("croqui-svg").innerHTML), "o ponto desapareceu");
});
t("sem perimetro e sem ponto a caixa apaga-se", () => {
  ev('(function(){ O.meta.lat=""; O.meta.lon=""; pintarCroqui(); })()');
