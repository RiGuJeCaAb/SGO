...
fontes                   areaGeoJSON, parseCSV, analisar, analisarCSV, meteoAutomatica,
                         analisarRelevo, detetarSensiveis, sugerirPT, obterAvisos
cartografia              photonEtiqueta (fonte primária), overpassEtiqueta e overpass
                         (recurso, 4 espelhos, POST), rumoEntre, tracarPerfil, pintarPerfil
meteograma               svgMeteo, metricas, resumoHoras, corRH, limiares
PCO                      pcoObj, pcoDef, funcoesExigiveis, prioridadeFuncao, pcoOptions,
                         renderPCO
pacote de canais         pacoteBase, fundirPacote, carregarCanais, guardarCanais, canalAdd,
                         optsCanal, pintarSel, pintarSelTodos, renderCatalogo, initCatalogo
...
PEA em vigor             horizonteValidade, baseVigor, controloMissoes, peaVigor,
                         divergencia, renderVigor
geração do PEA           llm, contexto, gerarPlan, gerarOps, retratoOperacional,
                         resumoRetrato, diferencasDesde, detPlan, detOps, emitirPEA
...
Setor: `{ estado, cmd, adj, ct, m, o, siresp, ba, tat, tatba, tip:[…] }` — `siresp`/`ba` são
o nível de manobra, `tat`/`tatba` o nível tático.
Ponto de trânsito: `O.dados.pt = { des, resp, ct, cd, obs }`, com acessor `ptObj()`, cartão
próprio na secção 2 e sugestão automática a partir da carta.
Perfil de elevação: `O.dados.perfil = { a, b, rot, total, e }`.
...
**Âmbito.** Cada canal tem `pasta` (nacional, distrito, subregiao, municipio, local) e `area`.
Um canal com `area` preenchida só existe quando a área bate com o distrito, o concelho ou a
sub-região da estação (`SUBREGIAO_ESTACAO`); fora disso `canalAplicavel()` devolve falso e o
canal desaparece da grelha de atribuição e de todas as listas, ficando no pacote assinalado
como fora de âmbito. Canais criados pelo fluxo "outro" nascem com pasta local e a área do
distrito do TO, para não migrarem para ocorrências noutra região.
...
(CC1 a CC5, CT1 a CT15, CM1 a CM30). Cento e um canais ao todo. Entradas do pacote levam
`pk:true`; as acrescentadas pelo utilizador não, e aparecem numa tabela própria, removíveis.
Há reposição do pacote, exportação e importação em JSON para o CSREPC distribuir um pacote
comum. `CANAIS.pv` guarda a versão do pacote: quando a revisão traz séries novas,
`fundirPacote()` acrescenta ao pacote gravado só o que falta, sem tocar no resto.
...
ativos; clicar atribui ou liberta. Havendo canais atribuídos numa rede, as listas de escolha
passam a mostrar apenas esses — o resto do pacote é ruído e fica de fora.
...
O seletor de funções é reconstruído a cada render: as funções já nomeadas saem da lista, salvo
as repetíveis (`FUNCOES_REPETIVEIS`), e as restantes são agrupadas e coloridas por prioridade
calculada em `prioridadeFuncao()` — essencial a vermelho quando exigível agora pela fase do SGO
ou pelo dispositivo, recomendada a amarelo quando a próxima fase ou um limiar próximo a
tornam previsível, de menor importância a verde nos restantes casos. As linhas das funções já
nomeadas levam a mesma marca de cor na margem.

## 9. O PEA em vigor — ciclo de vida

O PEA não é um documento fechado. Ao ser emitido, a proposta guarda três coisas além do texto:
`ts` e `validoTs` (relógio de vigência), `base` (a fotografia do dispositivo que fundamentou o
plano) e `ctrl` (as missões e propostas com estado de execução).

**Validade.** `horizonteValidade()` fixa o teto em 6 h, antecipado pelo fecho da janela
operacional ou pela próxima rotação de vento, com piso de 1 h para que o plano não nasça a
expirar.

**Divergência.** `divergencia(pea)` compara o estado atual com a `base` e devolve itens
ponderados, cada um com a sua situação e o fundamento doutrinário: reativação de setor e
mudança de fase do SGO valem 40; setor criado ou encerrado 25; alteração de estado de setor,
nível DECIR e dispositivo alterado em mais de 30 % valem 20; meios aéreos e rendições vencidas
15; máquinas de rasto, reserva, registos de evolução e plano de comunicações 10; ponto de
trânsito 5. A soma dá o veredito: em vigor, em vigor com divergências (20), revisão devida
(40) ou caducado, este último por esgotamento da validade.

**Controlo de execução.** Cada missão e cada proposta cicla entre por iniciar, em execução e
cumprida. A marcação entra na fita do tempo e a proposta seguinte lê a execução da anterior na
sua situação.

O cartão do PEA em vigor abre a secção 6 com o veredito, a barra de validade, a lista de
divergências e o controlo de execução, e repinta a cada alteração de estado e de 30 em 30
segundos. O motor de conformidade recebe um item próprio (`id:"vigor"`) que sobe a obrigação
quando a revisão é devida ou o plano caducou.
...
estado do PCO, verificações de conformidade, nível DECIR, e ainda o relógio de vigência, a base
de comparação e o controlo de missões descritos na secção anterior. As propostas são numeradas e cada
uma incorpora o que evoluiu desde a anterior (`evoDesdeUltimoPEA`).

A via determinística lê a situação operacional e não apenas a meteorologia.
`retratoOperacional()` devolve a contagem de setores por estado, meios, operacionais, meios
aéreos, máquinas de rasto, reserva, ponto de trânsito e tempos de empenhamento excedidos;
`diferencasDesde(anterior)` compara com a fotografia da proposta anterior e devolve a lista de
alterações. As propostas, o objetivo e a ação decisiva mudam com o estado dos setores — a
reativação de um setor passa à frente de tudo, a ausência de frentes ativas transita para
rescaldo e desmobilização faseada — e há propostas próprias para reserva por constituir, ponto
de trânsito por estabelecer, rendições vencidas e máquinas de rasto sem COPESP.

`metricas()` tolera a ausência de série meteorológica, devolvendo um objeto neutro com a nota
de que a análise está em falta, em vez de fazer falhar a emissão.
...
2. **Séries de banda alta CC, CT e CM.** Assentam na equivalência declarada entre os grupos
   SIRESP e os canais da ROB em simplex. Só o manobra 4 (CM4) tem confirmação direta, na
   DON n.º 2.
3. **Numeração da NEP n.º 8/NT/2010** para a banda alta de VHF, não verificada linha a linha.
4. **Organização das pastas do terminal SIRESP.** A estrutura implementada (nacional, distrito,
   sub-região, município, local) é uma leitura do âmbito operacional, não uma transcrição da
   árvore de pastas real dos terminais, que não foi possível obter em fonte aberta. Se houver
   acesso ao mapa de pastas em vigor, corrigir `PASTAS` e reclassificar as entradas do pacote.

---

## 12. Cartografia

A pesquisa de aglomerados, equipamentos sensíveis e candidatos a ponto de trânsito usa o
Photon `/reverse` como fonte primária, com `osm_tag` e `radius` em quilómetros: é o mesmo motor
que serve a geocodificação e a determinação do distrito, e responde nos ambientes onde os
espelhos do Overpass recusam o pedido — a aplicação corre em `file://`, o que basta para alguns
os rejeitarem, e as instâncias públicas limitam o número de consultas por origem.

O Overpass fica como recurso, percorrido por espelho até um responder, porque cobre objetos
sem nome que o Photon não indexa (parques de estacionamento, zonas industriais). A linha de
estado diz sempre qual das fontes respondeu, e quando ambas falham indica o erro de cada uma.

Nota de implementação: as listas são ordenadas por prioridade e distância, e a prioridade 0
existe — usar `x.prio===undefined? 9 : x.prio` e nunca `x.prio||9`, que faz a prioridade
máxima cair para o fundo da lista.
...
## 14. Método de trabalho esperado
...
- Briefing de passagem de comando gerado a partir da estrutura do PCO e do plano de
  comunicações.
- Ponto de trânsito no plano logístico impresso do PEA e no briefing.
- Perfil de elevação anexo ao PEA impresso, com o eixo de esforço traçado.
- Eixo do perfil definido por clique sobre o perímetro GeoJSON, em vez de rumo e distância.
- Gatilhos de revisão vindos da meteorologia em tempo real: cruzamento de HR abaixo de 30 %,
  rotação de vento observada e alteração de aviso do IPMA a entrarem na divergência.
- Anexo de controlo com o estado de execução das missões, para o COS assinar na passagem de
  comando.
